# MemoryFlow installer for Windows — one command to get started.
#
# Install CLI (default): connects to memoryflow.ai
#   irm https://raw.githubusercontent.com/nickkyjimmy/memoryflow-releases/main/install.ps1 | iex
#
# Self-host: starts a local MemoryFlow server + installs CLI + configures
#   $env:MEMORYFLOW_MODE="local"; irm https://raw.githubusercontent.com/nickkyjimmy/memoryflow-releases/main/install.ps1 | iex
#
...
# Configuration
# ---------------------------------------------------------------------------
$RepoUrl       = "https://github.com/nickkyjimmy/memoryflow.git"
$RepoWebUrl    = "https://github.com/nickkyjimmy/memoryflow"
...
# still valid for checking out self-host server assets from $RepoUrl.
$ReleaseRepo   = "nickkyjimmy/memoryflow-releases"
$DefaultInstallDir = Join-Path $env:USERPROFILE ".memoryflow\server"
$InstallDir    = if ($env:MEMORYFLOW_INSTALL_DIR) { $env:MEMORYFLOW_INSTALL_DIR } else { $DefaultInstallDir }
...
function Get-LatestVersion {
    try {
        $release = Invoke-RestMethod -Uri "https://api.github.com/repos/$ReleaseRepo/releases/latest" -ErrorAction Stop
...
function Get-SelfHostRef {
    if ($env:MEMORYFLOW_SELFHOST_REF) {
        return $env:MEMORYFLOW_SELFHOST_REF
    }
...
function Get-InstalledCliVersion {
    try {
        $firstLine = memoryflow version 2>$null | Select-Object -First 1
...
function Install-CliBinary {
    Write-Info "Installing MemoryFlow CLI from GitHub Releases..."

    if (-not [Environment]::Is64BitOperatingSystem) {
        Write-Fail "MemoryFlow requires a 64-bit Windows installation."
    }
    ...
    $url = "https://github.com/$ReleaseRepo/releases/download/$latest/memoryflow-cli-$version-windows-$arch.zip"
    $tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) "memoryflow-install"
    ...
    try {
        Invoke-WebRequest -Uri $url -OutFile (Join-Path $tmpDir "memoryflow.zip") -UseBasicParsing
    } catch {
    ...
    $checksumUrl = "https://github.com/$ReleaseRepo/releases/download/$latest/checksums.txt"
    try {
        ...
        $zipFile = Join-Path $tmpDir "memoryflow.zip"
        $actualHash = (Get-FileHash -Path $zipFile -Algorithm SHA256).Hash.ToLower()
        $releaseAsset = "memoryflow-cli-$version-windows-$arch.zip"
        $legacyAsset = "memoryflow_windows_$arch.zip"
    ...
    Expand-Archive -Path (Join-Path $tmpDir "memoryflow.zip") -DestinationPath $tmpDir -Force

    $binDir = Join-Path $env:USERPROFILE ".memoryflow\bin"
    if (-not (Test-Path $binDir)) {
        New-Item -ItemType Directory -Path $binDir -Force | Out-Null
    }

    $exeSrc = Join-Path $tmpDir "memoryflow.exe"
    if (-not (Test-Path $exeSrc)) {
        $exeSrc = Get-ChildItem -Path $tmpDir -Filter "memoryflow.exe" -Recurse | Select-Object -First 1 -ExpandProperty FullName
    }
    if (-not $exeSrc -or -not (Test-Path $exeSrc)) {
        Remove-Item $tmpDir -Recurse -Force
        Write-Fail "memoryflow.exe not found in downloaded archive."
    }

    Copy-Item $exeSrc (Join-Path $binDir "memoryflow.exe") -Force
    Remove-Item $tmpDir -Recurse -Force

    Add-ToUserPath $binDir
    Write-Ok "MemoryFlow CLI installed to $binDir\memoryflow.exe"
}
...
function Install-Cli {
    if (Test-CommandExists "memoryflow") {
        ...
        if ($isUpToDate) {
            Write-Ok "MemoryFlow CLI is up to date ($currentVer)"
            return
        }

        Write-Info "MemoryFlow CLI $currentVer installed, latest is $latestVer - upgrading..."
        Install-CliBinary

        $newVer = Get-InstalledCliVersion
        Write-Ok "MemoryFlow CLI upgraded ($currentVer -> $newVer)"
        return
    }

    Install-CliBinary

    if (-not (Test-CommandExists "memoryflow")) {
        Write-Fail "CLI installed but 'memoryflow' not found on PATH. Restart your terminal and try again."
    }
}
...
function Test-Docker {
    if (-not (Test-CommandExists "docker")) {
        Write-Fail @"
Docker is not installed. MemoryFlow self-hosting requires Docker and Docker Compose.

Install Docker Desktop for Windows:
  https://docs.docker.com/desktop/install/windows-install/

After installing Docker, re-run this script with `$env:MEMORYFLOW_MODE="local"`.
"@
    }
...
function Install-Server {
    Write-Info "Setting up MemoryFlow server..."
    ...
    if (Test-Path (Join-Path $InstallDir ".git")) {
        ...
        Write-Info "Cloning MemoryFlow repository..."
    ...
    Write-Info "Pulling official MemoryFlow images..."
    Pull-OfficialSelfHostImages
    Write-Info "Starting MemoryFlow services (this may take a few minutes on first run)..."
    ...
    if ($ready) {
        Write-Ok "MemoryFlow server is running"
    } else {
...
function Start-DefaultInstall {
    Write-Host ""
    Write-Host "  MemoryFlow - Installer" -ForegroundColor White
    Write-Host ""
    ...
    Write-Host "  [OK] MemoryFlow CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    Write-Host "  Next: configure your environment"
    Write-Host ""
    Write-Host "     memoryflow setup               " -NoNewline; Write-Host "# Connect to MemoryFlow Cloud (memoryflow.ai)" -ForegroundColor DarkGray
    Write-Host "     memoryflow setup self-host      " -NoNewline; Write-Host "# Connect to a self-hosted server" -ForegroundColor DarkGray
    Write-Host ""
    Write-Host "  Self-hosting? Install the server first:"
    Write-Host '     $env:MEMORYFLOW_MODE="with-server"; irm https://raw.githubusercontent.com/nickkyjimmy/memoryflow-releases/main/install.ps1 | iex'
    Write-Host ""
}
...
function Start-LocalInstall {
    Write-Host ""
    Write-Host "  MemoryFlow - Self-Host Installer" -ForegroundColor White
    ...
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host "  [OK] MemoryFlow server is running and CLI is ready!" -ForegroundColor Green
    Write-Host "  ============================================" -ForegroundColor Green
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "     memoryflow setup self-host  " -NoNewline; Write-Host "# Configure + authenticate + start daemon" -ForegroundColor DarkGray
    Write-Host ""
    ...
    Write-Host '     $env:MEMORYFLOW_MODE="stop"; irm https://raw.githubusercontent.com/nickkyjimmy/memoryflow-releases/main/install.ps1 | iex'
    Write-Host ""
}
...
function Start-Stop {
    Write-Host ""
    Write-Info "Stopping MemoryFlow services..."
    ...
    if (Test-Path $InstallDir) {
        ...
        Write-Warn "No MemoryFlow installation found at $InstallDir"
    }

    if (Test-CommandExists "memoryflow") {
        try {
            memoryflow daemon stop 2>$null
...
# Entry point
# ---------------------------------------------------------------------------
$mode = if ($env:MEMORYFLOW_MODE) { $env:MEMORYFLOW_MODE.ToLower() } else { "default" }
